...
import {
  ...
  HiOutlineLockClosed, HiOutlineArrowLeft, HiOutlineX, HiOutlineCheck,
} from 'react-icons/hi'
...
const STATUS_OPTIONS = [
  ...
  { value: 'CANCELLED', label: '❌ Cancelled',  desc: 'This trip was cancelled' },
]
...
export default function EditTripPage() {
  ...
  useEffect(() => {
    tripService.getTripById(id)
      .then((trip) => {
        ...
        if (trip.coverImage) {
          const src = trip.coverImage.startsWith('http')
            ? trip.coverImage
            : `${API_BASE}${trip.coverImage}`
          setCoverPreview(src)
        }
        setLoading(false)
      })
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await tripService.updateTrip(id, fd)
    ...
    return (
      <AppLayout>
        <div className="max-w-2xl mx-auto px-4 py-8 space-y-4 animate-pulse">
          ...
          <div className="h-7 bg-neutral-200 rounded w-1/2" />
    ...
    <AppLayout>
      <div className="max-w-2xl mx-auto px-4 sm:px-6 py-8 space-y-8">
        ...
        <div>
          ...
          <h1 className="font-display font-bold text-2xl text-neutral-900">Edit Trip</h1>
          <p className="text-sm text-neutral-500 mt-1">Update the details for your trip.</p>
        </div>
          ...
          <div>
            ...
            <div
              ...
              className={clsx(
                ...
                coverPreview ? 'border-transparent' : 'border-neutral-200 hover:border-primary-400 hover:bg-primary-50/30'
              )}
            >
              {coverPreview ? (
                <>
              ...
              {STATUS_OPTIONS.map((opt) => (
                ...
                >
                  <div className="flex-1">
                    <span className="text-sm font-semibold text-neutral-800 block">{opt.label}</span>
                    <span className="text-xs text-neutral-400">{opt.desc}</span>
                  </div>
                  {form.status === opt.value && (
                    <HiOutlineCheck className="w-4 h-4 text-primary-600 shrink-0" />
                  )}
                </button>
              ))}
            </div>
